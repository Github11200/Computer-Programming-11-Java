...
public class Hangman implements Runnable {
  ...
  static String getRandomWord(String[] array, int len) {
    ...
    for (int i = 0; i < 500; ++i) {
      if (array[randomIndex].length() >= len)
  ...
  static void viewStats(Scanner fromKey) {
    ...
    System.out.println("Winning Percentage: "
        + df.format((numberOfRoundsPlayed > 0 ? ((float) numberOfRoundsWon / numberOfRoundsPlayed) * 100 : 0.00))
        + "%");
    System.out.println("Losing Percentage: "
        + df.format((numberOfRoundsPlayed > 0 ? ((float) numberOfRoundsLost / numberOfRoundsPlayed) * 100 : 0.00))
        + "%");
    ...
    System.out.println("Guessing accuracy: "
        + (numberOfGuesses > 0 ? df.format((((float) numberOfRightGuesses / numberOfGuesses) * 100)) : 0.00) + "%");
  ...
  static void playRound(Scanner fromKey) {
    ...
    while (numberOfCharactersWrong != 6) {
      ...
      if (checkChar(guess, randomWord)) {
        ...
        System.out
            .println("CHARACTERS REVEALED: " + currentString + (!currentString.equals(randomWord) ? "\n\n\n" : ""));
  ...
  public static void main(String[] args) {
    ...
    Thread thread = new Thread(new Hangman());
    thread.start();
